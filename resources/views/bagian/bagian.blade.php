...
@section('content')
    @can('admin')
        <div class="flex justify-between items-center mb-2">
            ...
            <form action="/bagian/create" method="get">
                @csrf
                <button type="submit"
            ...
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    <th scope="col" class="px-6 py-3 text-center">
                        @if ($orderby != 'id')
                            <a href="/bagian?orderby=id&order=asc">
                            @elseif ($orderby == 'id' && $order == 'asc')
                                <a href="/bagian?orderby=id&order=desc">
                                @else
                                    <a href="/bagian">
                        @endif
                    ...
                    <th scope="col" class="px-6 py-3 text-center">
                        @if ($orderby != 'nama_bagian')
                            <a href="/bagian?orderby=nama_bagian&order=asc">
                            @elseif ($orderby == 'nama_bagian' && $order == 'asc')
                                <a href="/bagian?orderby=nama_bagian&order=desc">
                                @else
                                    <a href="/bagian">
                        @endif
                        <div class="flex items-center justify-center">
                            <h3 class="mr-2">Nama Bagian</h3>
                            @if ($orderby != 'nama_bagian')
                                <i class="fa-sharp fa-solid fa-sort"></i>
                            @elseif($orderby == 'nama_bagian' && $order == 'asc')
                                <i class="fa-sharp fa-solid fa-sort-up"></i>
                    ...
                    <th scope="col" class="px-6 py-3 text-center">
                        @if ($orderby != 'klasifikasi')
                            <a href="/bagian?orderby=klasifikasi&order=asc">
                            @elseif ($orderby == 'klasifikasi' && $order == 'asc')
                                <a href="/bagian?orderby=klasifikasi&order=desc">
                                @else
                                    <a href="/bagian">
                        @endif
                        <div class="flex items-center justify-center">
                ...
                @foreach ($bagians as $index => $bagian)
                    <tr
                        ...
                        <td class="px-6 py-4 text-center">
                            {{ $bagian->klasifikasi->klasifikasi }}
                        </td>
                        @can('admin')
                            <td class="px-6 py-4 text-center flex justify-center">
                                <a href="/bagian/{{ $bagian->id }}/edit"
